...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    const { startTimestamp, endTimestamp, market } = req.query;
    ...
    try {
        ...
        if (market && typeof market === "string") {
            url = `https://api.prod.flash.trade/pnl-info/by-market/${market}?startTimestamp=${startTimestamp}&endTimestamp=${endTimestamp}`;
        } else {
            url = `https://api.prod.flash.trade/pnl-info/all-logs?startTimestamp=${startTimestamp}&endTimestamp=${endTimestamp}`;
        }

        const response = await axios.get(url);
        const trades: Trade[] = response.data;

        const ownerPnlSummary: Record<string, { "Net PNL": number, "Gross PNL": number, "Total Fees Usd": number }> = {};

        for (const trade of trades) {
            const owner = trade.owner;
            const netPnlUsd = parseFloat(trade.netPnlUsd) * 10 ** -6;
            const totalFeesUsd = parseFloat(trade.totalFeesUsd) * 10 ** -6;
            const grossPnlUsd = netPnlUsd + totalFeesUsd;

            if (!ownerPnlSummary[owner]) {
                ownerPnlSummary[owner] = {
                    "Net PNL": 0,
                    "Gross PNL": 0,
                    "Total Fees Usd": 0
                };
            ...
            ownerPnlSummary[owner]["Net PNL"] += netPnlUsd;
            ownerPnlSummary[owner]["Gross PNL"] += grossPnlUsd;
            ownerPnlSummary[owner]["Total Fees Usd"] += totalFeesUsd;
        }

        res.status(200).json(ownerPnlSummary);
    } catch (error) {
        console.error("Handler error:", error);
